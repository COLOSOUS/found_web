...
class Pet:
    ...
    def save_pet1(self, data):
        ...
        sql="""
            INSERT INTO m_encontrada(Nombre,Correo,Region,Comuna,Celular,foto_id)
            VALUES(%s,%s,%s,%s,%s,%s)        
            """
        self.cursor.execute(sql,(*data[0:5], id_archivo))
        self.db.commit()

    def save_pet2(self, data):
        # procesar y guardar el archivo
        fileobj = data[5]
        filename = fileobj.filename
        ...
        sql = """
               INSERT INTO m_perdidas(Nombre,Correo,Region,Comuna,Celular,foto_id)
               VALUES(%s,%s,%s,%s,%s,%s)        
               """
        self.cursor.execute(sql, (*data[0:5], id_archivo))
        self.db.commit()
